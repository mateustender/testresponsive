    ...
    $.fn.jsonSuggest = function(searchData, settings) {
        ...
        return this.each(function() {
            ...
            function selectResultItem(item) {
                obj.val(item.text);
                $(results).html('').hide();
                app.events.fireEvent("hideSuggestDropdown");


                if (typeof settings.onSelect === 'function') {
                    settings.onSelect(item, obj);
                }
                app.events.fireEvent("addTag");
            }
            ...
            function showResultLayer(scope) {
                var inputEl = $(scope);
                $(results).css({
                    'top': (inputEl.position().top + inputEl.height() + 15) + 'px',
                    'left': inputEl.position().left + 'px'
                });
                $(results).show().css('height', 'auto');
                app.events.fireEvent("showSuggestDropdown");
            }

            // Build the results HTML based on an array of objects that matched
            // the search criteria, highlight the matches if feature is turned on in
            // the settings.
            function buildResults(resultObjects, sFilterTxt, scope) {
            ...
            function runSuggest(e) {
                ...
                if (settings.ajaxResults === true) {
                    searchData(this.value, settings.wildCard,
                            ...
                            settings.notCharacter, (function(resultObjects) {
                        if (typeof resultObjects === 'string') {
                            resultObjects = JSON.parse(resultObjects);
                        }
                        buildResults(resultObjects, sFilterTxt, this);
                    }).createDelegate(this));
